...
/* SELECT ELEMENTS */
		...
		function clickMe() {
			...
			activateClickSelection(".contactInfo", "no");
			activateClickSelection(".nameSection", "no");
			activateClickSelection(".workArea > div aside:nth-of-type(2)", "no");
		...
		$('#sortableActivate-btn').click( function() {
			...
			if (buttonState === 'enable' ) {
				...
				//make sortable
				$('aside ul, .mainWrapper ul, .myLearningWrapper, .joined').sortable({cancel: 'h2', opacity: 0.8}).css( 'cursor', 'pointer' );
...
/* THE OBJECTS */
	...
	function aboutMe(header, message) {
		...
		this.addAboutMe = function() {
			$('#forAboutMe, #aboutMe').show();
			if (header === '' && message === '') {
				$('#forAboutMe, #aboutMe').hide();
			} else {
				$('#aboutMe').append(
	...
	function myExperience(occupation, datesWork, employer, description) {
		...
		this.addMyExperience = function() {
			...
			var wrkExp = $('<div></div>').addClass('myExperience formFilledOut').append(
	...
	function mySkills(skillName, skillDescription) {
		...
		this.addMySkill = function() {
			...
			if (skillName !== '' && skillDescription !== '') {
				$('#mySkills ul').append(
					$('<li></li>').addClass('skillSection formFilledOut').append(
						$('<h5>' + this.skillName + '</h5>'),
						$('<p>' + this.skillDescription + '</p>')
					)
				);
			} else {
				if ( $('#mySkills').find('li.formFilledOut').length === 0 ) {
	...
	// skills section 2
	function mySkills2(skillName2) {
		this.skillName2 = skillName2;
		
		this.addMySkill2 = function() {
			$('#forSkills, #mySkills').show();
			if (skillName2 !== '') {
				$('#mySkills ul').append(
					$('<li>' + skillName2 + '</li>').addClass('formFilledOut')
				);
			} else {
				if ( $('#mySkills').find('li.formFilledOut').length === 0 ) {
					$('#forSkills, #mySkills').hide();
	...
	//////////////////////////////PERSONAL INFORMATION FORM//////////////////////////////////////
		...
		$('#updateTemplate').on('click', function() {
				...
				$('#mySkillsForm div').each(function() {
					var skillName2 = $(this).find('input').val();
					var toAddSkill2 = new mySkills2(skillName2);
					toAddSkill2.addMySkill2();
				});
				//add others
